...
-- I'm not clear on why this is "better", at least for the 2nd case of `safeTail`
--    --> Because it makes the "failure" case explicit with `Nothing`
-- --> See `safe` library for alternatives like `tailMay`, `tailDef`, etc.
-- --> See also `Data.List.NonEmpty`

safeTail :: [a] -> Maybe [a]
safeTail []       = Nothing
safeTail (x : []) = Nothing -- shouldn't this one return [] like `tail` does (and `tailMay`)?
                            -- --> the author prefers `Nothing` to `Just []`
