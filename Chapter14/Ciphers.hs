...
genWord :: Gen String
genWord = listOf1 $ elements ['a'..'z']     -- `listof1` for non-empty list!
...
caesarProp :: Property
caesarProp = forAll genCaesarParams
               (\(n, xs) -> (uncaesar n (caesar n xs)) == xs)
...
vigenereProp = forAll genVigenereParams
               (\(ks, xs) -> (unvigenere ks (vigenere ks xs)) == xs)
